...
const CONFIG = require("routes/config").S3BUCKET;
...
const webExecutables = /\.(aspx|asp|css|swf|xhtml|rhtml|shtml|jsp|js|pl|php|cgi|exe)$/i;
...
const BUCKETNAME = CONFIG["IMAGE"][process.env.ENV] ? CONFIG["IMAGE"][process.env.ENV] : "staging-uberclone";
 ...
 * @param {*} data
 * @param {*} _cb
 * @param {*} BUCKET_CONFIG // to store documents in public/private bucket
 * @returns
 */
const ACL = { 1: "public-read", 2: "private" };

async function generateSignedUrl(data, aclType = "1") {
    return new Promise((resolve, reject) => {
        const ACL_CONFIG = ACL[aclType];
        let bucketName = ACL[aclType] === "public-read" ? BUCKETNAME : CONFIG["PRIVATE"][process.env.ENV];

        const { file_name, mime_type, custom } = data;

        const file_timestamp = `_ts_${moment().unix()}`;
 ...
 *
 * @param {*} key
 * @returns temp link for document
 */
async function tempS3SignedUrl(fileName, aclType = "1") {
    const isPublicRead = ACL[aclType] === "public-read";
    const bucketKey = isPublicRead ? "DATA" : "PRIVATE";
    const BUCKETNAME = CONFIG[bucketKey][process.env.ENV];

    const params = { Bucket: BUCKETNAME, Key: fileName };

    if (!isPublicRead) params.Expires = Number(process.env.PUBLIC_URL_EXPIRY) || 60; // Default to 60 seconds

    try {
        return isPublicRead ? `${process.env.S3_BASE_URL}${fileName}` : s3.getSignedUrl("getObject", params);
    } catch (err) {
...
async function createFileAndUploadDataAsync(file_name, data) {
    try {
        var mime_type = mime.getType(file_name);
        var s3 = new AWS.S3();
        ...
        await s3
            .putObject(
                ...
                function (err) {
                    if (err) {
                        console.log("🚀 ~ file: s3-services.js:326 ~ createFileAndUploadDataAsync ~ err:", err);
                    } else {
                        uri = "https://" + BUCKETNAME + "." + s3.endpoint.host + "/" + file_name;
                    }
                }
            )
            .promise();
...
var uploadMulter = multer({
    ...
    limits: { fileSize: 100 * 1024 * 1024 }, // Set a file size limit of 100 MB
});
...
const multerStorage = multer({ storage: uploadMulterStorage, limits: { fileSize: 100 * 1024 * 1024 } });
...
function updatePrivateUrlsRecursively(obj) {
    const baseUrl = process.env.WEB_IMAGE_BASE_URL;
    const bucketName = CONFIG["PRIVATE"][process.env.ENV];
    const awsRegion = SECRET.AWS_REGION;
    let docBaseURL = `https://${bucketName}.s3.${awsRegion}.amazonaws.com/`;
    const regex = new RegExp(docBaseURL, "g"); // Create a global regex
    for (var key in obj) {
        if (typeof obj[key] === "object" && obj[key] !== null) {
            updatePrivateUrlsRecursively(obj[key]);
        } else if (typeof obj[key] === "string") {
            obj[key] = obj[key].replace(regex, `${baseUrl}#/fileAccess?fileAccessView=`);
        }
    }
}
...
module.exports.multerStorage = multerStorage;
...
module.exports.createFileAndUploadDataAsync = createFileAndUploadDataAsync;
module.exports.updatePrivateUrlsRecursively = updatePrivateUrlsRecursively;
module.exports.tempS3SignedUrl = tempS3SignedUrl;
